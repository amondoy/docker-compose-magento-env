...
RUN apt-get update && apt-get -y install lsb-release apt-transport-https ca-certificates apt-utils wget
RUN wget -O /etc/apt/trusted.gpg.d/php.gpg https://packages.sury.org/php/apt.gpg
RUN echo "deb https://packages.sury.org/php/ $(lsb_release -sc) main" | tee /etc/apt/sources.list.d/php.list

RUN apt-get update && apt-get -y install php7.4
RUN apt-get -y install php7.4-mysql php7.4-xml php7.4-intl php7.4-curl php7.4-bcmath php7.4-gd php7.4-mbstring php7.4-soap php7.4-zip composer
